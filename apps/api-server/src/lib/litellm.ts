...
export async function litellmUpdateUser(params: {
  ...
  // Only include fields the caller actually provided. Sending `models` on a
  // budget-only update would clobber the user's existing model allowlist (a
  // budget change must not silently reset which models they can call), and
  // sending `max_budget` on a models-only update would reset their ceiling.
  const body: Record<string, unknown> = { user_id: params.userId };
  if (params.maxBudget !== undefined) body.max_budget = params.maxBudget;
  if (params.allowedModels !== undefined) body.models = params.allowedModels;
